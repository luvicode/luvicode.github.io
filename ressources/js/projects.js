...
const modalpLink=document.querySelector('#modal--portfolio .plink');
    ...
    if(this.readyState==4 && this.status==200){
        ...
        btnBalises.forEach(btnBalise=>{
            btnBalise.addEventListener('click', (e)=>{
                ...
                let modalLinkhref= (modalLink.href=data['project'][idInt]['btn-link']);
                // si le l'anchor ne comporte pas de lien
                if(!modalLinkhref){
                    console.log("pas de lien");
                    //On supprime le btn
                    modalLink.style.display="none";
                    modalpLink.style.display= "block";
                    
                }else{
                    modalLink.style.display="block";
                    modalpLink.style.display="none";
                }
                ...
                closeBtnPortfolio.addEventListener('click', ()=>{
                    ...
                    // caclul des hauteurs des deux éléments avant le portfolio
                    const positionPortfolio = (document.querySelector('#home').clientHeight + document.querySelector('#services').clientHeight);
                    // scroll auto vers portfolio
